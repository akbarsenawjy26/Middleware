const DataPhasePower = require("../Model/Influx/phasePower");

const phasePowerData = async (req, res) => {
  const { slaveId } = req.params;
  try {
    const data = await DataPhasePower.fetchDataPhasePower(slaveId);
    // console.log("data phase power:", data);
    if (data.length === 0) {
      return res
        .status(404)
...
const phasePowerDataRange = async (req, res) => {
  const { slaveId } = req.params;
  ...
  try {
    const data = await DataPhasePower.fetchDataPhasePowerRange(
      slaveId,
      startDate,
      endDate
    );
    // console.log("data phase power range:", data);
